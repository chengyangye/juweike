	...
	<table class="table table-bordered">
			...
			<?php $__i=0; foreach ((array)$res as $r) { $__i++; ?>			 
			      <tr>
						...
						<span>
						[
						<a href="ggkadd-<?php echo $r->id; ?>.html">修改</a>
						]
						</span>
						<span>
						[
						...
						<span>
						[
						<a href="ggkList-<?php echo $r->id; ?>.html" >数据监测</a>
						]
						</span>
						</td>
				  </tr>
...
$(function() {
	$("#gotonext").click(function(){
		window.parent.$('.menu-dzp').trigger('click');
	});
});
...
function dellbs(id,o){
	if(id=='a'){
		if(confirm('确定删除这些活动信息吗？')){
			...
			ajax('ggk-del.html',{ id:ids.join(',')},function(){
				$('td').find('input[type="checkbox"]:checked').each(function(){
		...
		if(confirm('确定删除此条活动信息吗？')){
			ajax('ggk-del.html',{ id:id},function(){
