            ...
            <div class="row-fluid">
                <div class="span12">
                    <div class="box">
                        <div class="box-title">
                            <div class="span10">
                                <h3><i class="icon-edit"></i>关键字回复设置</h3>
                            </div>
                        </div>
                    </div>
                </div>
	...
	<form class="form-horizontal" id="reply_form" style="margin-left: 10px;margin-top:10px;">
			...
			<div class="control-group">
			  <label class="control-label" for="answertype" style="text-align: right;margin-top: -4px;">匹配类型:</label>
			    <div class="controls">
			    <select class="span2" id="pptype">
					  <option value="1">模糊匹配</option>
					  <option value="2">全匹配</option>
				</select>
				 <div>全匹配是对话完全一致才能触发，模糊匹配是是要包含就触发，两种匹配均区分大小写。</div>
			   </div>
...
$(function(){
	...
	//初始化数据
	if(itemData && itemData.pubsId){
		//alert($.toJSON(itemData));
		$("textarea[name='welcome']").val($.trim(itemData.replyContent));
		$contentDiv.html(Emotion.replaceEmoji($.trim(itemData.replyContent)));
		$('input[relval="'+itemData.resId+'"]').prop('checked',true);
		$('#ppgjz').val(itemData.keyWord);
		$('#pptype').val(itemData.replyType);
		if(itemData.replyType == '1'){
			 $("#answertype").val("0");
			 $('#answertype0').prop('checked',true);
			 $("div#textreply").show();
			 $("div#multiArticle11").hide();			 
			 initRenderArticle();
		 }else if(itemData.replyType != '1'){
			 $("#answertype").val('1');
			 $('#answertype1').prop('checked',true);
			 $("div#textreply").hide();
			 $("div#multiArticle11").show();
			 setselid(itemData.resId)
		 }
	}else{
		 $("#answertype").val("0");
		 $('#answertype0').prop('checked',true);
	...
	$("#submitbtn").click(function() {
		...
		//图文回复操作
			...
			if($v != ''){
				$.post('/admin/baseService/ajax2-<?php echo $ooid; ?>.html',{ type:1,con:$v,kw:$('#ppgjz').val(),pptyp:$('#pptype').val()},function(data) { 
					...
					if (data == -1) {
						...
						tusi("设置成功");
						setTimeout(function(){
							goto('/admin/baseService/keyword.html')
						},888);
					}else if(data == 2){
						tusi("设置成功");
						setTimeout(function(){
		...
		if (submitData.answertype == 0) {
			...
			$.post('/admin/baseService/ajax2-<?php echo $ooid; ?>.html',{ type:submitData.answertype,content:submitData.reply,kw:$('#ppgjz').val(),pptyp:$('#pptype').val()},function(data) { 
