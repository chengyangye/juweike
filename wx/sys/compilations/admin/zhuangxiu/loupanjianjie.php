					...
					<div class="box">
						...
						<div class="box-content">
							<form action="" method="post" id="lbsForm" class="form-horizontal form-validate" ><?php echo tk();  echo $m->hidden('id'); ?>
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_pic','micro_zhuangxiu_setpic');">选择封面</button>
								...
								<div class="control-group">
									<label class="control-label">装修公司头部图片：</label>
									<div class="controls">
									<img class="thumb_img" src="<?php echo $m->headpic; ?>" id="pic_headpic"  style="max-height:100px;" />
										<?php echo $m->hidden('headpic'); ?>
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_headpic','micro_zhuangxiu_setheadpic');">选择图片</button>
											<span class="help-inline">建议尺寸：宽720像素，高350像素</span>
										</span>
									</div>
								</div>
								<div class="control-group">
									<label class="control-label">成功案例头部图片：</label>
									<div class="controls">
									<img class="thumb_img" src="<?php echo $m->apartpic; ?>" id="pic_apartpic" style="max-height:100px;" />
									<?php echo $m->hidden('apartpic'); ?>									
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_apartpic','micro_zhuangxiu_setapartpic');">选择图片</button>
											<span class="help-inline">建议尺寸：宽720像素，高350像素</span>
										</span>
									</div>
								</div>
								...
								<div class="control-group">
									<label for="object_desc" class="control-label">装修公司简介：</label>
									<div class="controls">
								...
								<div class="control-group">
									<label for="traffic_desc" class="control-label">装修预约：</label>
									<div class="controls">
										<?php echo $m->select($yy_arr,'yyid'); ?>
									</div>
								</div>	
								<div class="control-group">
									<label for="traffic_desc" class="control-label">装修新闻：</label>
									<div class="controls">
										<?php echo $m->select($xw_arr,'xwid'); ?>
									</div>
								</div>
								<div class="control-group">
									<label for="traffic_desc" class="control-label">装修客户：</label>
									<div class="controls">
								...
								<div class="control-group">
									<label for="album_title" class="control-label">成功案例名称：</label>
									<div class="controls">
...
<br/><br/><br/></body>
	...
	var located = true;
	//定位坐标
	var destPoint = new BMap.Point($('#micro_zhuangxiu_setjd').val(),$('#micro_zhuangxiu_setwd').val());
	$(function(){		
		/**开始处理百度地图**/
		...
		map.addEventListener("click", function(e){
			if(confirm("确认选择这个位置？")){
				destPoint = e.point;
				$('#micro_zhuangxiu_setjd').val(destPoint.lng);
				$('#micro_zhuangxiu_setwd').val(destPoint.lat);
				map.clearOverlays();
		...
		function callback(posi){
			$("#micro_zhuangxiu_setaddr").removeAttr("disabled");
		...
		$("#positioning").click(function(){
			if($("#micro_zhuangxiu_setaddr").val() == ""){
				alert("请输地址！");
				return ;
			}
			...
			located = true;
			local.setMarkersSetCallback(callback);
			local.search($("#micro_zhuangxiu_setaddr").val());
			return false;
		});
