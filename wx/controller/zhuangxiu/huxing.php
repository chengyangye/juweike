...
if(Session::has('wxid') && Session::has('wid')){
	$set = new Model('micro_zhuangxiu_set');
	$set->find(array('wid'=>Session::get('wid')));
	if(!$set->has_id()){
		die();
	}
	$lp = new Model('micro_zhuangxiu_ziloupan');
	$zlpres = $lp->where(array('wid'=>Session::get('wid')))->order('sort desc,id')->list_all();
	$res = array();
	
	//查找户型
	$hxtype = new Model('micro_zhuangxiu_type');
...
function hasqj($hx){
	$qj = new Model('micro_zhuangxiu_type_full_view');
