...
G.logic.page = {
    ...
    common: function () {
        ...
        if ($talbecheckbox.length > 0) {
            $("table.ajax-checkbox input[type='checkbox'][name='show']").click(function () {
                var $this = $(this)
                var f = $this.attr("checked"), l = $talbecheckbox.attr("ajax-length"), id = $this.attr("data-id");
                var url = $talbecheckbox.attr("ajax-url");
                if (f) {
                    var ck_num = $("table.ajax-checkbox input[type='checkbox'][name='show']:checked").length;
                    if (l && l != 0) {
                        var tmp = "最多勾选{0}个";
        ...
        //$("[rel=tooltip]").length > 0 && $("[rel=tooltip]").tooltip();
    ...
    copy: function () {
        ZeroClipboard.setDefaults({
            moviePath: "/res/zeroclipboard.swf"
        });
        $(".copy_text").each(function (i) { 
            var $id = "copy_button{0}".format(i); 
            var _tmp = '  <button class="btn copy" id="{0}" type="button" data-clipboard-target="{2}" data-clipboard-text="{1}"><i class="icon-copy"></i>复制</button>  <span class="alert copy-success help-inline alert-success hide "  >复制成功,请粘帖到您需要的地方</span>';
            var $this = $(this), v = $this.text(), $iid = $this.attr("id"); 
