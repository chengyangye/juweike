...
$mu = new Model('user');
		...
		if(!empty($u->un) && !empty($u->pwd) && ($mu->is_real(array('un','pwd')) || $mu->is_real(array('email','pwd')) || $mu->is_real(array('telephone','pwd')))){			
			if($mu->isstop != '0'){
				...
				if(!$_SERVER['IS_OEM'] && (strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.zongyangtech.cn') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'ww.com') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.com') === false)){
...
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.zongyangtech.cn') !== false){
