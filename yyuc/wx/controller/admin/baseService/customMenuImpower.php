...
if($p->try_post()){
	...
	if(!empty($p->wun)&&!empty($p->wpwd)){
		...
		$http->post('/cgi-bin/login', array('f'=>'json','imgcode'=>trim($p->imgcode),'username'=>$p->wun,'pwd'=>md5($p->wpwd)));
		...
		if($sres==''){
			...
			if($reds->ErrCode==-6){//错误的用户名密码
				...
				//修改开发路径
				$http->post('/cgi-bin/callbackprofile?t=ajax-response&lang=zh_CN&token='.$token, array('url'=>'http://wx.zongyangtech.cn/mpapi.html?appid='.Session::get('wid'),'callback_token'=>md5(Session::get('wid').Conf::$management_center_target)));
				$http->getContent(true);
				sleep(1);
				//开启开发接口
				...
				$p->isval = '1';
				Redirect::delay_to('/admin/main.html',2);
			}
