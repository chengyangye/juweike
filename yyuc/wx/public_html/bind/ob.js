...
$(function() {
	...
	$(".main .rebind-btn").click(
			function() {
				location.href = "/admin/ai/bind?action=bind&rebind=true&wuid="
						+ $(".main .rebind-btn").attr("data-id")
			});
...
function setCenter(a) {
	var b = $(window).width(), d = $(window).height(), c = a.outerWidth(), e = a
			.outerHeight();
	if (b > c) {
		a.css("left", (b - c) / 2 + "px")
	} else {
...
function getStep() {
	$
			.post(
					"/admin/ai/bind?action=process&step=" + step,
					function(a) {
						if (a.invalid) {
							alert("您的公众账号绑定次数过于频繁，此公众账号已被系统冻结，如有疑问，请联系客服！");
							location.href = "/admin/main.jsp";
							return false
						}
						if (a.forbit) {
...
function loadImg() {
	$(".preload").one("load", function() {
		...
		loadPreImg()
	}).attr("src", "http://wxj.weixinjia.net/image/admin/bind/loading2.gif");
	loadSliderImg()
}
function loadPreImg() {
	var c = 4;
					...
					function() {
						...
						for ( var a = 1; a <= c; a++) {
							(function(g) {
								...
								f.attr("src",
										"http://wxj.weixinjia.net/image/admin/bind/big"
												+ g + ".jpg")
							})(a)
						}
					}).attr("src",
					"http://wxj.weixinjia.net/image/admin/bind/loading.gif")
}
function showBigTip() {
	$(".tip1").animate({
		opacity : 0
