...
G.ui.tips = {
    err: function (t, u) {
        this._set(t, u, {
            useOverlay: true, position: "center", icon: "error", autoclose: null, closeOverlay: true, buttons: {
                button1: {
                    text: '纭畾', danger: false, onclick: function () {
                        $.fallr('hide');
                    }
                }
            }
    ...
    confirm: function (t, u) {
        $.fallr('show', {
            buttons: {
                button1: {
                    text: '纭畾', danger: true, onclick: function () {
                        window.location = u;
                    }
                },
                button2: {
    ...
    confirm_flag: function (t, f) {
        $.fallr('show', {
            buttons: {
                button1: {
                    text: '纭畾', danger: true, onclick: f
                },
                button2: {
                    text: '鍙栨秷'
                }
    ...
    confirm_tips:function (u,t) {
        $.fallr('show', {
            ...
            buttons: {
                button1: {
                    text: '楠岃瘉', danger: true, onclick: function () {
                        window.location = u;
                    }
                },
                button2: {
                          ...
                          if (typeof KindEditor !== "undefined" && KindEditor.instances) {
                              $.each(KindEditor.instances, function () {
                                  ...
                                  if (r) {
                                      if (v.length == 0) {
                                          var msg = m;
                                          G.ui.tips.info(msg ? msg : "鍐呭涓嶈兘涓虹┖")
                                  ...
                                  if (e) {
                                      ...
                                      if (vv.length <= min || vv.length >= max) {
                                          G.ui.tips.info(em ? em : "鍐呭涓嶈兘灏忎簬{0}涓斿ぇ浜巤1}".format(min, max));
                                          _sb = false; return false;
                                      }
...
G.logic.page = {
    ...
    table: function () {
        ...
        //#region 鍏ㄩ€�
        var $allCheck = $("#listTable input.check_all"); // 鍏ㄩ€夊閫夋
        var $listTableTr = $("#listTable tr:gt(0)");
        var $idsCheck = $("#listTable input[name='check']"); // ID澶嶉€夋
    ...
    copy: function () { 
        ...
        $("span.copy_text").each(function (i) {
            ...
            var _tmp = '  <button class="btn copy" id="{0}" type="button" data-clipboard-text="{1}"><i class="icon-copy"></i>澶嶅埗</button>  <span class="alert copy-success help-inline alert-success hide "  >澶嶅埗鎴愬姛,璇风矘甯栧埌鎮ㄩ渶瑕佺殑鍦版柟</span>';
            ...
            if (((/^(https?|s?ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(v)))) {
                var _atmp = '<a href="{0}" target="_blank" title="鐐瑰嚮鏂扮獥鍙ｆ墦寮€">{0}</a>'
...
G.util.location = {
    ...
    locSelection: function (district) {
        var htm = ['<select name="province" data-rule-required="true"><option value="" >璇烽€夋嫨</option>'];
        $.each(G.util.area, function (pid, pinfo) {
            htm.push('<option value="' + pid + '">' + pinfo.n + '</option>');
        });
        htm.push('</select> <select name="city" data-rule-required="true"><option value="" >璇烽€夋嫨</option></select> <select name="district" data-rule-required="true"><option value="" >璇烽€夋嫨</option></select>');
        ...
        p.change((function (_, __, ___) {
            return function () {
                ...
                var cHtm = ['<option value="">璇烽€夋嫨</option>'];
                if (G.util.area[pid]) {
        ...
        c.change((function (_, __, ___) {
            return function () {
                ...
                var dHtm = ['<option value="">璇烽€夋嫨</option>'];
...
G.logic.uploadify = {
    ...
    live: function () {
        ...
        $("a.item_close").live("click ", function (e) {
            ...
            $.fallr('show', {
                    ...
                    button1: {
                        text: '纭畾', danger: true, onclick: function () {
                            ...
                            if ((_self.op.count - $("li.imgbox").length) >= 0) {
                                ...
                                button.html('<i class="icon-plus-sign"></i> 缁х画涓婁紶');
                            }
                ...
                content: '<p>浣犵‘瀹氳鍒犻櫎杩欏紶鍥剧墖鍚楋紵</p>',
                icon: 'trash'
            });
        });
        $('.ipost-list').sortable({
    ...
    add: function (data) {
        var _tmp = ' <li class="imgbox" data-post-id="{0}" data-url="{3}">'
           + '<a class="item_close" href="javascript:void(0)" title="鍒犻櫎"></a> '
           ...
           + ' <textarea name="imagestexts[{0}][]" class="bewrite" cols="3" rows="4" style="resize: none" data-rule-maxlength="150" placeholder="鍥剧墖鎻忚堪...">{2}</textarea>'
    ...
    set: function () {
        ...
        _self.op.el.uploadify({
            ...
            'buttonText': '<i class="icon-plus-sign"></i> 娣诲姞鍥剧墖',
            ...
            'queueSizeLimit': 100,
            'uploadLimit': c <= 0 ? 1 : c,
            'onUploadError': function (file, errorCode, errorMsg, errorString, queue) { alert(file.name + "涓婁紶澶辫触!") },
            'onUploadStart': function (file) {
                $('#file_upload-button').html('<i class="icon-plus-sign"></i> 缁х画涓婁紶');
            },
            'onInit': function (instance) {
                if ((_self.op.count - $("li.imgbox").length) <= 0) {
                    var button = $("#file_upload-button");
                    button.addClass("disabled").attr("style", "z-index: 999;")
                    button.html('涓婁紶宸茶揪闄愬埗...');
                }
            },
