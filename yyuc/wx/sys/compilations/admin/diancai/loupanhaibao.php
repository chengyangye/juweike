            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <div class="box">
                        <div class="box-title">
                            <div class="span7">
                                <h3><i class="icon-table"></i>订单信息 </h3>
                            </div>
                            ...
                            <div class="row-fluid dataTables_wrapper">
                                <form action="/plus/formajax.php" method="post" class="form-horizontal">
                                    <table id="listTable" class="table table-bordered table-hover  dataTable">
                                        ...
                                        <tbody>
                                        <?php foreach ((array)$ddres as $dd) {  ?>
                                        <tr>
                                        <td><?php echo $dd->time; ?></td>
                                        <td>
                                        <?php foreach ((array)explode(";",$dd->goodsData) as $nn) {
										$a = explode(',',$nn);
										if(!empty($a[1]))
										{
										echo $cc[$a[0]]."-".$a[1]."份<br>";
										}	}									
										?>
                                        </td>
                                        <td>
...
<?php }  } ?>
	...
	function use_hotel(id,o){
		if(confirm('确定标记使用此条记录吗？')){
			ajax('loupanhaibao-bj.html',{ id:id,s:3},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function con_hotel(id,o){
		if(confirm('确定标记确认此条记录吗？')){
			ajax('loupanhaibao-bj.html',{ id:id,s:1},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function rej_hotel(id,o){
		if(confirm('确定标记拒绝此条记录吗？')){
			ajax('loupanhaibao-bj.html',{ id:id,s:2},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function del_hotel(id,o){
		if(confirm('确定删除此条记录吗？')){
			ajax('loupanhaibao-bj.html',{ id:id,s:'del'},function(m){
				tusi('删除成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
