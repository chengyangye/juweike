					...
					<div class="box">
						...
						<div class="box-content">
							<form action="" method="post" id="transform" class="form-horizontal form-validate"><?php echo tk();  echo $m->hidden('id'); ?>
								...
								<div class="control-group">
									<label for="area" class="control-label">特色鲜花描述：</label>
									<div class="controls">
								...
								<div class="control-group">
									<label for="description" class="control-label">特色鲜花介绍：</label>
									<div class="controls">
									<?php echo $m->textarea('jianjie','class="input-large" style="height:80px;width:380px;"'); ?>	
									</div>
								</div>
								<div class="control-group">
									<label for="prices" class="control-label">特色鲜花图片：</label>
...
<script>		
		...
		$(function(){
			$('#transform').submit(function(){
				...
				$('#micro_huadian_typepic').val($.toJSON(pics));
			});
			var val = $('#micro_huadian_typepic').val();
			if($.trim(val)==''){
				val = '[]';
			}
