					...
					<div class="box">
						...
						<div class="box-content">
							<form action="" method="post" id="transform" class="form-horizontal form-validate"><?php echo tk();  echo $m->hidden('id'); ?>
								...
								<div class="control-group">
									<label for="category_id" class="control-label">选择子楼盘：</label>
									<div class="controls">
								...
								<div class="control-group">
									<label for="floor_num" class="control-label">楼层：</label>
									<div class="controls">
										<?php echo $m->text('louceng','class="input-large"  required="required"'); ?>
										<span class="maroon">*</span>
										<span class="help-inline">如：3-10</span>
									</div>
								</div>
								<div class="control-group">
									<label for="area" class="control-label">建筑面积：</label>
									<div class="controls">
										<?php echo $m->text('mianji','class="input-large"  required="required"'); ?>
										<span class="maroon">*</span>
										<span class="help-inline">如：约103平方米</span>
									</div>
								</div>
								<div class="control-group">
									<label for="fang" class="control-label">房屋户型：</label>
									<div class="controls">
									<?php echo $m->select($fang,'fang','class="input-mini"  required="required"'); ?>													
										房
									<?php echo $m->select($fang,'ting','class="input-mini"  required="required"'); ?>													</select>
										厅
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									<label for="description" class="control-label">户型介绍：</label>
									<div class="controls">
									<?php echo $m->textarea('jianjie','class="input-large" style="height:80px;width:380px;"'); ?>	
									</div>
								</div>
								<div class="control-group">
									<label for="prices" class="control-label">户型图片：</label>
...
<script>		
		...
		$(function(){
			$('#transform').submit(function(){
				...
				$('#micro_estate_typepic').val($.toJSON(pics));
			});
			var val = $('#micro_estate_typepic').val();
			if($.trim(val)==''){
				val = '[]';
			}
