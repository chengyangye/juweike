					...
					<div class="box">
						...
						<div class="box-content">
							<form action="" method="post" id="lbsForm" class="form-horizontal form-validate" ><?php echo tk();  echo $m->hidden('id'); ?>
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_pic','micro_zhengwu_setpic');">选择封面</button>
								...
								<div class="control-group">
									<label class="control-label">政务部门头部图片：</label>
									<div class="controls">
									<img class="thumb_img" src="<?php echo $m->headpic; ?>" id="pic_headpic"  style="max-height:100px;" />
										<?php echo $m->hidden('headpic'); ?>
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_headpic','micro_zhengwu_setheadpic');">选择图片</button>
											<span class="help-inline">建议尺寸：宽720像素，高350像素</span>
										</span>
									</div>
								</div>
								<div class="control-group">
									<label class="control-label">服务类型头部图片：</label>
									<div class="controls">
									<img class="thumb_img" src="<?php echo $m->apartpic; ?>" id="pic_apartpic" style="max-height:100px;" />
									<?php echo $m->hidden('apartpic'); ?>									
										<span class="help-inline">
											<button class="btn select_img" type="button" onclick="setpic('pic_apartpic','micro_zhengwu_setapartpic');">选择图片</button>
											<span class="help-inline">建议尺寸：宽720像素，高350像素</span>
										</span>
									</div>
								</div>
								...
								<div class="control-group">
									<label for="object_desc" class="control-label">政务部门简介：</label>
									<div class="controls">
										<?php echo $m->texteditor('xiangmu',1,'600px','100px'); ?>
									</div>
								</div>
								<div class="control-group">
									<label for="traffic_desc" class="control-label">特别提醒：</label>
									<div class="controls">
										<?php echo $m->texteditor('jiaotong',1,'600px','100px'); ?>
									</div>
								</div>								
								<div class="control-group">
									<label for="traffic_desc" class="control-label">政务预约：</label>
									<div class="controls">
										<?php echo $m->select($yy_arr,'yyid'); ?>
									</div>
								</div>	
								<div class="control-group">
									<label for="traffic_desc" class="control-label">政务新闻：</label>
									<div class="controls">
										<?php echo $m->select($xw_arr,'xwid'); ?>
									</div>
								</div>
								<div class="control-group">
									<label for="traffic_desc" class="control-label">政务市民：</label>
									<div class="controls">
								...
								<div class="control-group">
									<label for="album_title" class="control-label">服务类型名称：</label>
									<div class="controls">
...
<br/><br/><br/></body>
	...
	var located = true;
	//定位坐标
	var destPoint = new BMap.Point($('#micro_zhengwu_setjd').val(),$('#micro_zhengwu_setwd').val());
	$(function(){		
		/**开始处理百度地图**/
		...
		map.addEventListener("click", function(e){
			if(confirm("确认选择这个位置？")){
				destPoint = e.point;
				$('#micro_zhengwu_setjd').val(destPoint.lng);
				$('#micro_zhengwu_setwd').val(destPoint.lat);
				map.clearOverlays();
		...
		function callback(posi){
			$("#micro_zhengwu_setaddr").removeAttr("disabled");
		...
		$("#positioning").click(function(){
			...
			located = true;
			local.setMarkersSetCallback(callback);
			local.search($("#micro_zhengwu_setaddr").val());
			return false;
		});
