            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <div class="box">
                        <div class="box-title">
                            <div class="span7">
                                <h3><i class="icon-table"></i>预约试驾订单信息 </h3>
                            </div>
                            <div class="span5">
                                <form action="yysjrecord-<?php echo $h->id; ?>.html" method="get" class="form-horizontal">
                                    <?php echo $m->date('form2','class="input-small-z datepick" placeholder="预定日期"'); ?>
                        ...
                        <div class="box-content">
                            ...
                            <div class="row-fluid">
                                <div class="span12 control-group">
                                    <div class="alert">
                                       预约试驾总数：<span class="redamount"><?php echo $ddzs; ?></span>个　　预约成功：<span class="redamount"><?php echo $ddcg; ?></span>个　　预约失败：<span class="redamount"><?php echo $ddsb; ?></span>个　　等待确认：<span class="redamount"><?php echo $dddd; ?></span>个
                                    </div>
                                </div>
                            ...
                            <div class="row-fluid dataTables_wrapper">
                                <form action="/plus/formajax.php" method="post" class="form-horizontal">
                                    <table id="listTable" class="table table-bordered table-hover  dataTable">
                                        ...
                                        <tr>
                                        <td><?php echo $ind; ?></td>
                                        <td><?php echo $pinpai_arr[$dd->ppid]; ?></td>
                                        <td><?php echo $cx_arr[$dd->xid]; ?></td>
                                        <td><?php echo $cxing_arr[$dd->xingid]; ?></td>
                                        <?php $__i=0; foreach ((array)$ddxjson as $d) { $__i++; ?>
                                        <?php $val = 'form'.($__i-1); ?>
                                        <td>
                                        <?php echo $dd->$val; ?>
                                        ...
                                        <a href="javascript:;" onclick="rej_hotel(<?php echo $dd->id; ?>,this);">标记放弃</a>
...
<?php }  } ?>
	...
	function use_hotel(id,o){
		if(confirm('确定标记使用此条记录吗？')){
			ajax('yysjrecord-bj.html',{ id:id,s:3},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function con_hotel(id,o){
		if(confirm('确定标记确认此条记录吗？')){
			ajax('yysjrecord-bj.html',{ id:id,s:1},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function rej_hotel(id,o){
		if(confirm('确定标记拒绝此条记录吗？')){
			ajax('yysjrecord-bj.html',{ id:id,s:2},function(m){
				tusi('标记成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
	...
	function del_hotel(id,o){
		if(confirm('确定删除此条记录吗？')){
			ajax('yysjrecord-bj.html',{ id:id,s:'del'},function(m){
				tusi('删除成功');
				setTimeout(function(){
					goto(location.href);
				},1000);				
