	...
	<div class="content-box-content">
		...
		<div class="tab-content default-tab" >
			<table>
				<thead>
					<tr>
			<th style="padding-left:5px;border-left:1px solid #C1DAD7">序号</th><th>账号</th><th>名称</th><th>类型</th><th>余额(￥)</th><th>手机</th><th>QQ</th><th>所属员工(代理)</th><th>状态</th><th>操作</th>
					</tr>
				</thead>
				<tbody>
				<?php $__i=0; foreach ((array)$res as $r) { $__i++; ?>
					...
					<td>
					<?php echo $zt_arr[$r->isstop]; ?>
					</td>
					<td>
					<a href="javascript:tochongzhi(<?php echo $r->id; ?>);">充值</a>&nbsp;|&nbsp;					
					<a href="daililiebiao-<?php echo $r->id; ?>-<?php echo md5($r->id.$mu->pwd); ?>.html">下级代理</a>&nbsp;|&nbsp;
					<a href="zuizhongliebiao-<?php echo $r->id; ?>-<?php echo md5($r->id.$mu->pwd); ?>.html">下级用户</a>&nbsp;|&nbsp;
...
function stop(id,o){
	if(id=='a'){
		...
		if(confirm('确定停用这些代理吗？')){
			ajax('daililiebiao-stop.html',{ id:ids.join(',')},function(data){
				if(data == 'ok'){
					tusi('操作成功');
					setTimeout(function(){
						goto(location.href);
...
function tochongzhi(id){
	pophtml('<iframe id="jd_iframe" src="dailichongzhi-'+id+'.html" scrolling="auto" frameborder="0" width="370" height="185"></iframe>',410,265);
}
</script>