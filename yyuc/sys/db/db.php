...
class DB{
	...
	public function init($host,$port,$username,$password,$dbname){
		...
		if (!$this->conn = @mysql_connect($host.':'.$port, $username, $password)){
			die('<center>MYSQL链接建立失败,请进入<a href="'.Conf::$http_path.Conf::$management_center_target.'">开发管理中心</a>配置数据库连接</center>');
		}
	...
	public function base_query($sql,$pam=null) {
		if(is_array($sql)){
			...
			if(Conf::$need_debug_log){
				Debug::log_sql_begin(self::$current_db.':'.Request::ip().'::'.$sql);
			}
			//数据验证的执行
			if(self::$dbdes  !== null){
				self::$dbdes->db = $this;
			...
			$this->statement = @mysql_query($sql, $this->conn);			
			if(Conf::$need_debug_log){
				Debug::log_sql_end();
			}
			if (!$this->statement) {
				$mysqlerr = mysql_error();
				if(is_array($mysqlerr)){
					$mysqlerr = implode(',', $mysqlerr);
				}
				Log::error($mysqlerr."：".$sql);
			}
			return $this->statement;
		}else{
			 if(is_string($pam)){//参数为字符串类型
			 	$sql = str_replace('@yYUc_SqL@',"'".mysql_escape_string($pam)."'",$sql);
			 	...
			 	for($i=0;$i<$sqllen;$i++){
			 		if($pam[$i]===null){
			 			...
			 			$sql.=($sqlss[$i]."'".mysql_escape_string($pam[$i])."'");
			 		}
			 	}
			 	$sql.= $sqlss[$sqllen];
			 }
	 ...
	 */
	function query($sql,$pam=null,$callback = null){
		$res = array();
		if(is_string($sql)){
			$sql = str_replace('?', '@yYUc_SqL@', $sql);
		}
		...
		if(is_callable($callback)){
			while($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
				call_user_func_array($callback,array($tn));
			}
		}else{
			while($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
				$res[] = $tn;
	 ...
	 */
	function query_model($sql,$pam=null,$callback = null){
		$res = array();
		if(is_string($sql)){
			$sql = str_replace('?', '@yYUc_SqL@', $sql);
		}
		...
		if(is_callable($callback)){
			while($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
				call_user_func_array($callback,array((object)$tn));
			}
		}else{
			while($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
				$res[] = (object)$tn;
	...
	function one_column_array($sql,$pam=null){
		$query = $this->base_query($sql,$pam);
		$res = array();
		while($tn=mysql_fetch_array($query,MYSQL_NUM)){
			$res[] = $tn[0];
		}		
		if(count($res)>0){
			return $res;
	...
	function fill_one_model(& $model,$condition,$pam=null,$ord=null,$limit=null){
		...
		$query = $this->base_query($sql);
		if($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
			foreach ($tn as $k=>$v){
				$model->$k = $v;
			}
			return true;
	...
	function get_fields_array($table){
		if(empty(self::$tablefieldsarray[$table])){
			self::$tablefieldsarray[$table] = $this->query("show full fields from `$table`");
		}
