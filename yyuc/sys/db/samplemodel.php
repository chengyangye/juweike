...
class SampleModel{
	...
	public function val_tel($field,$errmsg=null){
		$this->val_reg($field,"/(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/", $errmsg===null?YYUC::i18n('validate.tel'):$errmsg);
	}
	/**
	 * 字段国内手机合法性验证
	 * @param string $field 字段名称
	...
	public function val_phone($field,$errmsg=null){
		$this->val_reg($field,"/^0{0,1}(13[0-9]|15[0-9]|18[0-9])[0-9]{8}$/", $errmsg===null?YYUC::i18n('validate.phone'):$errmsg);
	}
	/**
	 * 字段Email格式合法性验证
	 * @param string $field 字段名称
	...
	function vercode($local,$width=120,$height=45,$attrs='',$ischinese=false,$codenum=4){
		...
		$html = '<input type="text" required="required" placeholder="点击输入" name="'.$elname.'" id="'.$id.'" '.$attrs.'/>';
		$html .= '<script>';
	 ...
	 * @return string 标签html字串
	 */
	public function select($array,$name='',$attrs='',$autoindex = null,$autotext = true){
		$additionstr = '"';
		if(is_string($array)){
			$attrs = $name;
			$name = $array;			
		...
		$autotextstr = '';
		if($autotext){
			$autotextstr ='yy_autotext="'.$this->elname('SEL_TXT_'.$name).'"';
		}
	...
	public function mulselect($tn,$name_arr,$attrs='',$addwhere='',$impstr='',$autotext = true){
		...
		foreach ($name_arr as $index=>$name){
			...
			if($index<($arr_len-1)){
				...
				$change = '"YYUC_getnextsel(this,\''.String::encryption($ntn).'\',\''.String::encryption($addwherestr).'\')"';
	 ...
	 * @return string 标签html字串
	 */
	public function checkbox($array,$name='',$attrs=''){
		if(is_string($array)){
			$attrs = $name;
			$name = $array;			
			if(is_array($attrs)){
			...
			foreach ($array as $k=>$v){
				$k=strval($k);
				$tag.='<input type="checkbox" name="'.$this->elname($name).'[]" value="'.htmlspecialchars($k).'" '.((in_array($k, $value,true))? 'checked="checked"' : '').' '.$attrs.'/>'.$v.'&nbsp;&nbsp;';
			}
			return $tag;
		}
	 ...
	 * @return string 标签html字串
	 */
	public function radio($array,$name='',$attrs=''){
		if(is_string($array)){
			$attrs = $name;
			$name = $array;			
			$array = $this->field_data($name);
		...
		foreach ($array as $k=>$v){
			...
			$tag.='<input type="radio" name="'.$this->elname($name).'" value="'.htmlspecialchars($k).'" '.(($k==$value)? 'checked="checked"' : '').' '.$attrs.'/>'.$v.'&nbsp;&nbsp;';
		}
		return $tag;
	}
	/**
	...
	function texteditor($name,$level=3,$width='640px',$height='300px'){
		...
		if(is_integer($level)){
			...
			$opts = '{langType:"'.$_SERVER['HTTP_ACCEPT_LANGUAGE'].'",allowImageUpload:'.$allowupload.',allowFlashUpload:'.$allowupload.',allowMediaUpload:'.$allowupload.',allowFileManager:'.($level==4||$level==7?'true':'false').',items:kindeditor_item'.($level==1?'0':intval($level/5+1)).'}';
		}else {
			$opts = '{langType:"'.$_SERVER['HTTP_ACCEPT_LANGUAGE'].'",'.$level.'}';
		}
