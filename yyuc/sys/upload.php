...
<input name="YYUC_UPLOAD_file" style="position: absolute;height: 100%;width:{$width}px;right: 0;z-index:2" type="file"/>
<input src="{$backgroundImg}" type="{$btntype}" style="display:block; position: absolute;height: 100%;width:100%;z-index:1;line-height: {$lineheight}px;font-size:{$fontSize}px;color:{$fontColor};text-align: center;" value="{$text}"/>
</span>
...
//开启图片上传后剪裁
	if(is_array($cutpic)){
$htm .= <<<EOT
	window.{$ttid}_uppimg = $('<img id="{$ttid}_realpic" style="position: absolute;left:-10000px;top:-10000px" src="'+url+'"/>');
...
$htm .= <<<EOT
		...
		ajax('{$surl}',window.{$ttid}_form.serialize()+'&'+$.param(window.{$ttid}_picpos),function(m){
			if(m!='no'){
...
function {$ttid}_failure(size,type){
	if({$onfailure}){
		if(type==2){
			...
			{$onfailure}('文件上传发生未知错误',$('#ttid'),4,'{$ttid}');
		}
		clearTimeout(window.{$ttid}jdproc);
		$('#{$ttid}_bfb').html('');
		$('#{$ttid}_name').html('');
 ...
 */
static function save_upload_file($subpath, $funupload=null,$funcut=null){
	//临时目录清空
	File::remove_dir(YYUC_FRAME_PATH.YYUC_PUB.'/upload/temp/',(time()-86400));
	File::creat_dir_with_filepath(YYUC_FRAME_PATH.YYUC_PUB.'/'.$subpath);	
	if(!empty($_FILES)&&isset($_FILES['YYUC_UPLOAD_file'])){
		...
		if($f['size']>$thissize){
			Response::write('<script>window.parent.'.$thisid.'=1;window.parent.'.$thisid.'_failure('.($f['size']/1000).',2);</script>');
		}else {
			$path_parts = pathinfo($f['name']);
			$filename = $subpath.'.'.$path_parts['extension'];
			if(stripos($filename, '.php')!==false){
				die();
			}
			if (!move_uploaded_file($f['tmp_name'], YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename)) {
				Response::write('<script>window.parent.'.$thisid.'=1;window.parent.'.$thisid.'_failure("server error",4);</script>');
			}else{
				if($funupload !==null){
					call_user_func_array($funupload,array(YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename));
				}
				echo('<script>window.parent.'.$thisid.'=1;window.parent.'.$thisid.'_success("/'.$filename.'","'.$filename.'");</script>');
			}
		}
	}else if(Request::post('key')){
		//剪切
		$filename = Request::post('key');
		if(!file_exists(YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename)){
			return false;
		}
		//剪切之后的处理
		$img_r = imagecreatefromstring(file_get_contents(YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename));
		if($img_r===false){
			Response::write('no');
		}
		$dsw = Request::post('w');
		...
		$filename2 = $subpath .'.jpg';
		imagejpeg($dst_r,YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename2,90);
		if($filename2!=$filename){
			unlink(YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename);
		}
		if($funcut !==null){
			call_user_func_array($funcut,array(YYUC_FRAME_PATH.YYUC_PUB.'/'.$filename2));
		}
...
private static function _resave_upload_file($key,$folderpath=null,$isFile = false){
	if(trim($key)==''){
		...
		//删除上次的文件地址
		@unlink(YYUC_FRAME_PATH.YYUC_PUB.'/'.substr($key, 4));
		return false;
	}elseif (strpos($key, '@-@')!==false){
		//修改后删除上次的文件地址
		$keys = explode('@-@', $key);
		$key = $keys[0];
		@unlink(YYUC_FRAME_PATH.YYUC_PUB.'/'.$keys[1]);
	}
	if($folderpath===null){
		$folderpath = 'upload/auto/'.date('Y/m/d',time());
	}
	if(!$isFile){
		File::creat_dir(YYUC_FRAME_PATH.YYUC_PUB.'/'.$folderpath);
		$tsubppath = '/'.$folderpath.'/'.basename($key);
		$newpath = YYUC_FRAME_PATH.YYUC_PUB.'/'.$folderpath.'/'.basename($key);
	}else{
		File::creat_dir_with_filepath(YYUC_FRAME_PATH.YYUC_PUB.'/'.$folderpath);
		$tsubppath = '/'.$folderpath;
		$newpath = YYUC_FRAME_PATH.YYUC_PUB.'/'.$folderpath;
	}	
	@unlink($newpath);
	rename(YYUC_FRAME_PATH.YYUC_PUB.'/'.$key, $newpath);
	return str_replace('//', '/',$tsubppath);
}
