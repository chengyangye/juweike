...
class Index{
	...
	public function _trans() {
		if(Conf::$is_developing||!@file_exists($this->com_path)){
			...
			$jadapter = Page::asset('@system/js/').'yyucadapter.js';
			$appdenstr = '</title><script type="text/javascript">var yyuc_jspath = "'.Page::asset('@system/').'";</script><script type="text/javascript" src="'.$jquery.'"></script><script type="text/javascript" src="'.$jadapter.'"></script>';
			
			/**
			//加载Html5音视频扩展 此处用在单独扩展用
			if(stripos($comRes, '</audio>')!==false||stripos($comRes, '</video>')!==false){
				$jahtml5av = Page::asset('@system/media/').'mediaelement-and-player.min.js';
				$jahtml5avcss = Page::asset('@system/media/').'mediaelementplayer.min.css';
				$appdenstr.='<script type="text/javascript" src="'.$jahtml5av.'"></script>';
				$appdenstr.='<link rel="stylesheet" href="'.$jahtml5avcss.'" />';				
			}
			*/
