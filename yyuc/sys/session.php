...
class Session{	
	...
	public static function get($k,$def=null){
		return isset($_SESSION[Conf::$session_prefix.$k]) ? $_SESSION[Conf::$session_prefix.$k] : $def;
	}
	...
	public static function once($k,$v=null){
		if($v === null){
			$_SESSION[Conf::$session_prefix.'YYUC_ONCE_'.$k] = null;
			unset($_SESSION[Conf::$session_prefix.'YYUC_ONCE_'.$k]);
		}else{
			$_SESSION[Conf::$session_prefix.'YYUC_ONCE_'.$k] = $v;
		}
	}
	...
	public static function flush($k){
		if(isset($_SESSION[Conf::$session_prefix.'YYUC_ONCE_'.$k])){
			$msg = $_SESSION[Conf::$session_prefix.'YYUC_ONCE_'.$k];
			self::once($k);
			return $msg;
		}		
		return null;
